...
const OnSiteTimeAndDateForm = ({ visitInfo, setVisitInfo, handleError, handleBack }: Props) => {
  ...
  const handleTimeSlots = async () => {
    try {
      /* const response = await GetTimeSlots(
        visitInfo.doctor.id || 0,
        visitInfo.service.id || 0,
        chosenDate.date,
      );
      */
...
export default OnSiteTimeAndDateForm;
